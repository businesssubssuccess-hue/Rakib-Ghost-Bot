...
// 👻 Ghost Net — Dynamic FCA Loader (Rakib)
let login;
try {
        ...
        if (_fs.existsSync(_activeFile)) {
                _activeName = (JSON.parse(_fs.readFileSync(_activeFile, "utf8")).active || "fca1").trim();
        }
        const _fcaDir = _path.join(process.cwd(), "fca-modules", _activeName);
        if (!_fs.existsSync(_fcaDir)) throw new Error("FCA module not found: " + _activeName);
        login = defaultRequire(_fcaDir);
        let _info = {};
        try { _info = JSON.parse(_fs.readFileSync(_path.join(_fcaDir, "info.json"), "utf8")); } catch { }
        console.log(`👻 [Ghost Net] Loaded FCA: ${_activeName} → ${_info.name || _activeName}`);
} catch (e) {
        console.log("⚠️ FCA loader fallback to bundled fb-chat-api:", e.message);
...
const titles = [
        ...
        [
                "G H O S T  N E T  B O T  v2@" + currentVersion
        ],
        [
                "GHOST NET BOT"
        ]
...
let subTitle = `👻 Ghost Net Edition | GoatBot V2@${currentVersion} | All Commands by Rakib Islam`;
const subTitleArray = [];
...
const author = ("👑 Bot Owner & Developer: Rakib Islam (Acs Rákíb)");
const srcUrl = ("🌐 Facebook: https://www.facebook.com/acs.rakib  |  Ghost Net Edition");
const fakeRelease = ("⚡ 558+ Commands | Prefix: .  |  Bengali + English  |  Ghost Net 👻");
                ...
                // is cookie string
                        ...
                        // is json (cookies or appstate)
                                ...
                                appState = appState
                                        .map(item => {
                                                let expiresStr;
                                                if (item.expirationDate && !isNaN(Number(item.expirationDate))) {
                                                        expiresStr = new Date(Number(item.expirationDate) * 1000).toUTCString();
                                                } else if (item.expires && item.expires !== "undefined") {
                                                        const d = new Date(item.expires);
                                                        expiresStr = isNaN(d.getTime()) ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toUTCString() : d.toUTCString();
                                                } else {
                                                        expiresStr = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toUTCString();
                                                }
                                                return {
                                                        ...item,
                                                        domain: "facebook.com",
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        try {
                                ...
                                const _activeFile = _path.join(process.cwd(), "fca-modules", "active.json");
                                const _activeFca = JSON.parse(_fs.readFileSync(_activeFile, "utf8")).active || "fca1";
                                const _infoFile = _path.join(process.cwd(), "fca-modules", _activeFca, "info.json");
                                const _info = JSON.parse(_fs.readFileSync(_infoFile, "utf8"));
                                log.info("FCA MODULE", `${_activeFca} → ${_info.name} | Anti-ban: ${_info.antiban} | Speed: ${_info.speed}`);
                        } catch {}
                        // ———————————————————— GBAN ————————————————————— //
                        let dataGban;
                        ...
                        if (global.GoatBot.config.serverUptime.enable == true && !global.GoatBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
                                ...
                                let nameUpTime;
                                try {
                                        const _replDomain = (process.env.REPLIT_DOMAINS || "").split(",")[0].trim();
                                        nameUpTime = `https://${_replDomain ?
                                                _replDomain :

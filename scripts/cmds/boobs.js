// Boobs — 18+ — Rakib Islam / Ghost Net Edition
...
module.exports = {
  config: {
    name: "boobs",
    aliases: ["tits18", "boobsnsfw"],
    version: "1.0",
    author: "Rakib Islam",
    countDown: 5,
    role: 1,
    shortDescription: { en: "NSFW boobs image 🔞" },
    category: "18+",
    guide: { en: "{p}boobs" }
  },
  onStart: async function ({ message }) {
    ...
    try {
      const { data } = await axios.get("https://hmtai.hatsunia.moe/v2/nsfw/boobs", { timeout: 10000 });
      const imgUrl = data.url;
      if (!imgUrl) throw new Error("No image");
      const resp = await axios.get(imgUrl, { responseType: "arraybuffer", timeout: 15000 });
      const ext = imgUrl.split(".").pop().split("?")[0] || "jpg";
      const out = path.join(__dirname, "cache", `boobs_${Date.now()}.${ext}`);

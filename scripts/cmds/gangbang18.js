// Gangbang18 — 18+ — Rakib Islam / Ghost Net Edition
...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 1,
    shortDescription: { en: "NSFW image 🔞" },
    category: "18+",
    guide: { en: "{p}gangbang18" }
  },
  onStart: async function ({ message }) {
    await fs.ensureDir(path.join(__dirname, "cache"));
    try {
      const { data } = await axios.get("https://hmtai.hatsunia.moe/v2/nsfw/gangbang", { timeout: 10000 });
      const imgUrl = data.url;
      if (!imgUrl) throw new Error("No image");
      const resp = await axios.get(imgUrl, { responseType: "arraybuffer", timeout: 15000 });
      const ext = imgUrl.split(".").pop().split("?")[0] || "jpg";
      const out = path.join(__dirname, "cache", `gb18_${Date.now()}.${ext}`);

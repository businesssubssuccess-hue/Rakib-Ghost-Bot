// Ass18 — 18+ — Rakib Islam / Ghost Net Edition
...
module.exports = {
  config: {
    name: "ass18",
    aliases: ["buttnsfw", "ass_18"],
    version: "1.0",
    author: "Rakib Islam",
    countDown: 5,
    role: 1,
    shortDescription: { en: "NSFW ass image 🔞" },
    category: "18+",
    guide: { en: "{p}ass18" }
  },
  onStart: async function ({ message }) {
    ...
    try {
      const { data } = await axios.get("https://hmtai.hatsunia.moe/v2/nsfw/ass", { timeout: 10000 });
      const imgUrl = data.url;
      if (!imgUrl) throw new Error("No image");
      const resp = await axios.get(imgUrl, { responseType: "arraybuffer", timeout: 15000 });
      const ext = imgUrl.split(".").pop().split("?")[0] || "jpg";
      const out = path.join(__dirname, "cache", `ass18_${Date.now()}.${ext}`);

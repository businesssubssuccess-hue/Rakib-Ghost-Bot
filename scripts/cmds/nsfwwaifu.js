// NSFW Waifu — 18+ — Rakib Islam / Ghost Net Edition
...
module.exports = {
  config: {
    name: "nsfwwaifu",
    aliases: ["waifunsfw", "waifu18"],
    version: "1.0",
    author: "Rakib Islam",
    countDown: 5,
    role: 1,
    shortDescription: { en: "NSFW Waifu anime image 🔞" },
    category: "18+",
    guide: { en: "{p}nsfwwaifu" }
  },
  onStart: async function ({ message }) {
    ...
    try {
      ...
      const out = path.join(__dirname, "cache", `nsfwwaifu_${Date.now()}.${ext}`);
      await fs.writeFile(out, Buffer.from(resp.data));
      await message.reply({ body: "🔞 NSFW Waifu | Ghost Net 18+", attachment: fs.createReadStream(out) });

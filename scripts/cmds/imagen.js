...
module.exports = {
  config: {
    ...
    guide: {
      en: "{pn} <prompt>\nExample: ${prefix}imagen futuristic dragon flying in space",
    },
  },
  ...
  onStart: async function ({ message, event, args, api, commandName }) {
    ...
    try {
      ...
      const url = `${apiBase}/api/imagen?prompt=${encodedPrompt}`;
      const imgPath = path.join(__dirname, "cache", `imagen_${event.senderID}.png`);

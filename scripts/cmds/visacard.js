...
module.exports = {
  config: {
    name: "visacard",
    aliases: ["fakevisa", "visa"],
    version: "1.0",
    author: "Rakib",
    countDown: 5,
    role: 0,
    shortDescription: "Animated নিয়ন Fake Visa Card",
    longDescription: "নাম দিয়ে একটা animated neon Visa card বানিয়ে দেবে (fake — only for fun)",
    category: "image",
    guide: { en: "{p}visacard <name> | reply | mention" }
  },
  ...
  onStart: async function ({ message, event, args, api }) {
    ...
    // Generate fake Visa card
    const cardNumber = generateVisa();
    ...
    const out = path.join(cacheDir, `visacard_${event.threadID}.gif`);
    ...
    const neonCycle = ["#00f0ff", "#ff00aa", "#00ff66", "#ffd700", "#ff5500", "#9d00ff"];
    const FRAMES = 12;
    ...
    for (let f = 0; f < FRAMES; f++) {
      ...
      ctx.fillText("PLATINUM ▸ NEON SERIES", cx + 30, cy + 55);

      // VISA logo (top-right) — large gold + glow
      ctx.font = "italic bold 52px Arial";
      ctx.textAlign = "right";
      ctx.fillStyle = "#ffd700";
      ctx.shadowColor = "#ffd700"; ctx.shadowBlur = 25;
      ctx.fillText("VISA", cx + cw - 30, cy + 60);
      ctx.shadowBlur = 0;

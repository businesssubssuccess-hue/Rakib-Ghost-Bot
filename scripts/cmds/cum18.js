// Cum18 — 18+ — Rakib Islam / Ghost Net Edition
...
module.exports = {
  ...
  onStart: async function ({ message }) {
    ...
    try {
      const { data } = await axios.get("https://api.waifu.pics/nsfw/cum", { timeout: 10000 });
      const resp = await axios.get(data.url, { responseType: "arraybuffer", timeout: 15000 });
      const ext = data.url.split(".").pop().split("?")[0] || "jpg";
      const out = path.join(__dirname, "cache", `cum18_${Date.now()}.${ext}`);

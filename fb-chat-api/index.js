...
function buildAPI(globalOptions, html, jar) {
    ...
    // Detect all checkpoint types — block, numeric ID (/checkpoint/150109.../), login verify
    ...
    const isCheckpoint = checkpointPatterns.some(p => html.includes(p)) ||
        /\/checkpoint\/\d+\//.test(html);
    if (isCheckpoint) {
        return log.error('login', "👻 Account checkpoint এ আছে! Browser এ login করে checkpoint solve করুন, তারপর নতুন cookie export করুন।", 'error');
    }
    if (!userCookie && !tiktikCookie) {
        return log.error('login', "👻 Cookie invalid! c_user পাওয়া যাচ্ছে না। Cookie Editor দিয়ে fresh cookie export করুন।");
    }
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    mainPromise = mainPromise
        ...
        .then(res => {
            ...
            if (!Obj) {
                // buildAPI returns undefined on checkpoint/invalid cookie — convert to callback error
                return callback(new Error("👻 Login failed: cookie মেয়াদ শেষ বা account checkpoint এ আছে। Browser এ login করে সমাধান করুন এবং নতুন cookie export করুন।"));
            }
            ctx = Obj.ctx;
            api = Obj.api;
            return res;
